...
const OUTPUT_DIR = join(PROJECT_DIR, "experiment-logic-split-output");
const AGENT_DIR = join(PROJECT_DIR, "experiment-logic-split-agent");

// Split model: Opus meta-agent writes prompts for Haiku task-agent
const OPUS_CONFIG = {
...
const config: RunConfig = {
  ...
  llm: {
    diagnosis: OPUS_CONFIG,
    modification: OPUS_CONFIG,      // Meta agent runs on Opus (smart enough to write good prompts)
    evaluation: HAIKU_CONFIG,       // Task agent runs on Haiku (weaker, more room to improve)
  },
  budget: {
...
async function main() {
  ...
  console.log(`  Meta model: ${OPUS_CONFIG.model}`);
  console.log(`  Task model: ${HAIKU_CONFIG.model}`);
